...
module.exports = {
    ...
    execute(message,args){
        ...
        var rolename = serverdata.RankUpdatePermission.split(",");
        ...
        if(args[0] == undefined) return message.reply(discordmodule.InfoEmbed("사용법",";진급 <유저 닉네임>"));
        message.reply(discordmodule.InfoEmbed("로딩중","잠시 기다려주세요")).then((msg)=>{
            roblox.getIdFromUsername(args[0]).then((userid)=>{
                roblox.changeRank(serverdata.DefaultGroup,userid,1).then((result)=>{
                    message.client.channels.cache.get(serverdata.RankUpdateLogChannel).send(new discord.MessageEmbed().setTitle("진급 로그").setAuthor(message.author.tag,message.author.avatarURL()).setDescription(`<@${message.author.id}>이(가) **${args[0]}**플레이어의 랭크를 **${result.oldRole.name}**에서 **${result.newRole.name}**로 변경했습니다.`).setColor("#7bff6f"))
                    ...
                    if(error == "Error: 403 You do not have permission to manage this member."){
                        return msg.edit(discordmodule.ErrorEmbed("오류","봇이 올릴 수 있는 권한 이상의 계급입니다."));
